...
functional revision is `18d45140e5466251e28c8a50feeab2d5f34aa3b6`; it adds Linux registered-VFS
write-failure and synchronization-failure rejection evidence on top of uncommitted transaction crash rollback, including
...
The pinned Core also injects an `xSync` failure through that registered VFS and verifies a typed
persistence error, baseline preservation, and absence of the transient row after reopen. This
remains bounded registered-VFS evidence rather than arbitrary third-party or physical power-loss
qualification.
...
through Core migration/open; existing sidecars are inspected again afterward and immediately before
the hydrated storage is published, so a changed identity fails closed across the startup boundary.
SQLite-created sidecars that were absent at preflight are still required to be private regular files.
Post-publish runtime replacement and non-default SQLite VFS behavior remain outside the current
claim.
